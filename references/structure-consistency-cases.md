...
- Common mistake: showing a shallow round water tray in the exploded view but changing it into side water pockets, rectangular tanks, or an unrelated support in the section view.
...
- View consistency: if the section view shows a charcoal fire chamber inside a raised base with vent holes, the exploded view should keep the pot/chimney/base shell as one main vessel assembly, then separate only the fire grate, ash tray, charcoal basket, lid/cap, water-filled safety tray, or insulation tray when justified. When a user-provided water tray is included, keep the tray's shallow round/oval tray geometry consistent between exploded and section views, with the water surface sitting inside that same tray. Do not add an alcohol-lamp-style water tray to a charcoal version unless the user provides that detail or the object visibly has one.
